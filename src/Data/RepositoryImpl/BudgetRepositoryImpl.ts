...
export class BudgetRepositoryImpl implements IBudgetRepo {
  ...
  async getByMonth(
    ...
    const budgets: CategoryBudgetLimit[] = storedBudgets
      ? JSON.parse(storedBudgets)
      : [];
    return budgets.filter((budget) => {
      const extractedYearAndMonth = budget.month.split("-", 2);
      return (
        extractedYearAndMonth[0] === String(year) &&
        extractedYearAndMonth[1] === String(month).padStart(2, "0")
      );
    });
  }
  async save(budgetLimit: CategoryBudgetLimit): Promise<void> {
    ...
    return await this.dataSource.storeItem(
      StorageKeys.budgets,
  ...
  async delete(categoryId: string): Promise<void> {
    ...
    await this.dataSource.storeItem(
      StorageKeys.budgets,
